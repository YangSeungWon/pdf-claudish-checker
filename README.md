...
기준은 실측으로 잡았습니다. 사람이 쓴 CHI 논문 두 편이 **3.37 / 2.07**(옅음),
Claude가 쓴 것처럼 만든 문단이 **142.86**(짙음) 이었습니다.
...
- 요약의 갈래(칩)를 누르면 그 갈래만 걸러 봅니다. 설정은 따로 없습니다.
...
`lower bound` `matched` 처럼 **Claude 문체이기도 하고 정상적인 기술 용어이기도 한**
낱말들이 있습니다. 이런 것도 그냥 잡습니다. 끄는 스위치는 두지 않았습니다 —
일단 짚어 놓고 사람이 판단하는 쪽이 낫기 때문입니다.

그래도 안 잡는 것은 있습니다.

- **`-aware` `-agnostic` `-driven` `-only` `-facing`.** 굳어진 표현이 너무 많습니다.
- **`scaffolding` `tension between` `trajectory`(운동 맥락).** 학습과학·HCI의 정식 용어입니다.
- **논문이 스스로 정의한 이름.** `Cue-First`, `Delay-First` 같은 조건 이름처럼 대문자로
  시작하거나 문서 전체에서 3개 문장을 넘겨 되풀이되는 하이픈 합성어는 그 글의 용어로
  봅니다. 무엇을 뺐는지는 결과에 그대로 적습니다 (`이 글이 쓰는 용어로 보고 뺀 것: ...`).
...
갈래마다 예문이 있는지, 그 예문을 실제로 잡는지, 정상적인 연구 문장을 잘못
잡지는 않는지 봅니다. **갈래에 예문이 하나도 없으면 실패로 처리합니다** —
`assistant` 갈래가 어느 목록에도 들어 있지 않아 통째로 시험된 적이 없었고,
그 안에서 어순이 굳어 있어 자연스러운 문장을 못 받는 패턴을 한동안 못 봤습니다
(`Here's a quick rundown of the three conditions for you` 를 놓쳤습니다).
...
- 오탐이 있습니다. `X-level`(`participant-level` 등)을 넓게 잡기로 한 결과가 가장
  ...
  웹에서는 요약의 갈래(칩)를 눌러 그 갈래만 보면 됩니다.
